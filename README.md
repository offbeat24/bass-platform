...
### 새 NAN 프로젝트를 BASS와 함께 생성

BASS 저장소에서 다음 명령을 실행하면 새 폴더 생성, 현재 BASS package 고정,
의존성 설치, BASS 공통 설정과 NAN 2026 preset 초기화를 한 번에 수행한다.
...
npm ci
npm run build
npm run bass -- create /path/to/game-project --design
```

생성된 프로젝트에는 `tools/bass-platform-0.1.1.tgz`, `package.json`,
`package-lock.json`, `bass.yaml`, `nan2026.yaml`, `.bass/`, `nan/`과 agent
shim이 함께 생긴다. 대상 폴더가 이미 내용을 가지고 있으면 기존 파일 보호를 위해
중단하며, 그 경우 아래의 기존 프로젝트 연결 절차를 사용한다. NAN 적용이 필요 없는
공통 프로젝트는 `--preset none --profiles common,web`처럼 명시한다.
