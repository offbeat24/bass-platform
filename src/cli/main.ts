...
program
  ...
  .description("새 프로젝트 폴더를 만들고 로컬 BASS package 설치와 선택한 preset init을 수행")
  .option("--name <name>", "프로젝트 이름 (생략 시 대상 폴더 이름)")
  .option("--profiles <list>", "프로파일 목록 (쉼표 구분)", "common")
  .option("--preset <preset>", "none | nan2026", "none")
  ...
  .action((directory, opts) => {
    ...
    if (preset === "nan2026" && !profiles.includes("nan2026")) {
      profiles.push("nan2026");
    }
...
program
  ...
  .action((opts) => {
    ...
    const profiles = String(opts.profiles).split(",").map((s: string) => s.trim());
    if (opts.preset === "nan2026" && !profiles.includes("nan2026")) {
      profiles.push("nan2026");
    }
    const result = initProject({
      projectRoot: process.cwd(),
      name: opts.name ? String(opts.name) : path.basename(process.cwd()),
      profiles,
      owner: opts.owner,
      withDesign: Boolean(opts.design),
