...
3. Run `<BASS> runtime doctor <id>` before scaffold. Recommendation is advisory; require explicit runtime and destination selection before scaffold or install.
4. Managed files use checksums. Stop on conflict and preserve user edits.
5. Run `<BASS> runtime install` only with explicit approval. Then run `<BASS> runtime verify` only for selected targets.
