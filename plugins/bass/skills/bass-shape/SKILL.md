...
6. Split delivery into tasks with acceptance, exclusions, dependencies, owned paths, rollback, and required evidence. Create each with `<BASS> task new <task-id> --title <title> --if-missing`, then fill its contract. Do not create parallel tasks whose paths overlap.
