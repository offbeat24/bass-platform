...
`setup` preserves existing files. It appends or refreshes only the marked block in `AGENTS.md`, creates Claude/Cursor shims only for selected adapters, creates missing PRODUCT/TECH/DESIGN files without replacing existing specifications, and ignores `.bass/cache/` and `.bass/local.yaml`. A malformed marker is a conflict and stops integration without `--force`.
